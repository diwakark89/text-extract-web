        ...
        for (const selector of revealButtonSelectors) {
            try {
                ...
                for (const btn of buttons) {
                    ...
                    const mightReveal = text.includes('show') || 
                                       text.includes('more') || 
...
async function handleRevealButtonClick(selector) {
    try {
        ...
        targetElements.forEach(element => {
            ...
            element.querySelectorAll('button, a, .btn, [role="button"], [aria-expanded="false"]').forEach(btn => {
                relevantButtons.push(btn);
            });

            // Look for buttons in the parent element
            if (element.parentElement) {
                element.parentElement.querySelectorAll('button, a, .btn, [role="button"], [aria-expanded="false"]').forEach(btn => {
            ...
            if (element.parentElement) {
                Array.from(element.parentElement.children).forEach(sibling => {
                    if (sibling !== element) {
                        sibling.querySelectorAll('button, a, .btn, [role="button"], [aria-expanded="false"]').forEach(btn => {
                            if (!relevantButtons.includes(btn)) {
                                relevantButtons.push(btn);
                            }
                        });
        ...
        const revealButtonsToClick = relevantButtons.filter(btn => {
            const text = btn.textContent?.toLowerCase() || '';
            const hasRevealText = ['show', 'more', 'reveal', 'expand', 'view', 'see all', 'read', 'answer'].some(keyword => text.includes(keyword));
