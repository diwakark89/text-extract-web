 ...
 */
async function extractTextFromSelectors(selectors, options = { includeHiddenText: true }) {
    try {
        ...
        for (const selector of selectors) {
            try {
                ...
                const additionalHiddenText = Array.from(parentElements).map(parent => {
                    // Get all hidden elements that aren't part of our original selection
                    const hiddenChildren = Array.from(parent.querySelectorAll('*')).filter(child => {
                        const computedStyle = window.getComputedStyle(child);
                        return computedStyle.display === 'none' && !Array.from(elements).includes(child);
                    });

                    return hiddenChildren.map(hidden => extractHiddenText(hidden)).filter(t => t).join('\n');
                }).filter(text => text.length > 0);
...
async function attemptToRevealHiddenContent(elements) {
    try {
        ...
        const buttons = Array.from(document.querySelectorAll('button, a, .btn, [role="button"], .show-more, .read-more, .expand'));
        const relevantButtons = [];

        // Find buttons that might reveal content near our elements
        elements.forEach(element => {
            // Check for buttons within the element
            element.querySelectorAll('button, a, .btn, [role="button"]').forEach(btn => {
                relevantButtons.push(btn);
            });

            // Check for buttons that are siblings
            if (element.parentElement) {
                element.parentElement.querySelectorAll('button, a, .btn, [role="button"]').forEach(btn => {
        ...
        const revealButtons = relevantButtons.filter(btn => {
            ...
            const hasRevealText = ['show', 'more', 'reveal', 'expand', 'view', 'see all', 'read'].some(keyword => text.includes(keyword));
            const hasRevealAttr = btn.getAttribute('aria-expanded') === 'false' || 
...
function extractHiddenText(element) {
    ...
    if (isHidden) {
        ...
        if (!hiddenContent) {
            hiddenContent = element.innerHTML
